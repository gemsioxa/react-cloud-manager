...
const Item = (props) => {
    ...
    return (
        <>
            <ContextMenuTrigger id={`google_item_${props.item.name}`}>
                <div 
                    ...
                    draggable={true}
                >
                    {props.item.mimeType.split('.').pop() === 'folder' ? <FolderIcon /> : <FileIcon />}
                    <span className={ContentCss.contentItemTitle}>
                        {props.item.title}
                    </span>
                </div>
            </ContextMenuTrigger>
            <ContextMenu style={isDrag ? {display: 'none'} : ''} className={ContentCss.context} id={`google_item_${props.item.name}`}>
...
export default function Content() {
    ...
    const onClickDelete = (path) => {
        api.google.deleteResource(id, path)
            .then(() => {
                setIsLoadingNewPath(true);
            });
    };
    ...
    const onClickSubmitRename = () => {
        ...
        api.google.replaceResource(id, renameData)
            .then(() => {
                setIsLoadingNewPath(true);
            })
            .then(() => {
    ...
    const onClickDownload = (data) => {
        api.google.getResourceDownload(id, data.path)
            .then((resp) => {
    ...
    const onClickSubmitCreateFolder = () => {
        ...
        api.google.putResource(id, createFolderData)
            .then(() => {
                setIsLoadingNewPath(true);
            })
            .then(() => {
    ...
    const onEndDrop = (value) => {
        ...
        api.google.replaceResource(id, data)
            .then(() => {
                setIsLoadingNewPath(true);
            })
    };
    ...
    return (
        ...
        >
            ...
            <ContextMenuTrigger id='google_container' className={ContentCss.contentContext}>
            ...
            <ContextMenu className={ContentCss.context} id='google_container'>
